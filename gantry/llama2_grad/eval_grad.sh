...
#gantry run --beaker-image seungjuh/open-instruct-public-240711 \
gantry run --beaker-image gantry run --beaker-image seungjuh/open-instruct-public-240711 \
  --venv base --name $NAME-evaluation --cluster ai2/pluto-cirrascale --workspace ai2/safety --pip requirements.txt --gpus $NUM_GPUS --priority high --preemptible --env-secret WANDB_API_KEY=WANDB_API_KEY --env-secret HF_TOKEN=HUGGING_FACE_HUB_TOKEN --env WANDB_PROJECT=llama2-evaluation --env WANDB_ENTITY=seungjuhan3 --env WANDB_NAME=$NAME-evaluation --env-secret OPENAI_API_KEY=openai_api_key --budget ai2/oe-adapt \
