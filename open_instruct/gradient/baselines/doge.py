...
class GradientTrackerDoge:
        ...
        if len(self.temporary_gradient_storage) >= self.batch_size:
            ...
            for i, dataset_id in enumerate(self.temporary_dataset_ids):
                ...
                if valid_num_batches is not None:  # validation grads
                    if self.eval_gradient_store.get(dataset_id) is None:
                        self.eval_gradient_store[dataset_id] = [projected_vectorized_grads[i]]
                    else:
                        self.eval_gradient_store[dataset_id].append(projected_vectorized_grads[i])

                else:
                    if self.train_gradient_store.get(dataset_id) is None:
                        self.train_gradient_store[dataset_id] = [projected_vectorized_grads[i]]
                    else:
                        self.train_gradient_store[dataset_id].append(projected_vectorized_grads[i])
    ...
    def calc_sim(self, gradient_store_avg: dict[int, torch.tensor]):
        ...
        for train_id, val_id in itertools.product(range(num_train_set), range(num_valid_set)):
            try:
                train_grad = torch.stack(self.train_gradient_store[train_id], dim=0).mean(dim=0)
                val_grad = torch.stack(gradient_store_avg[val_id], dim=0).mean(dim=0)
...
def test_model(args,
               ...
               embedding_size: int,
               device: torch.device,
               # gradient_tracker: GradientTrackerDoge,
               ):
    # model.eval()
    # assert len(gradient_tracker.eval_gradient_store) == 0, "gradient_store_avg should be empty when testing."
    total_eval_loss = 0
    ...
    # gradient_store_avg = gradient_tracker.eval_gradient_store
...
def main():
    ...
    gradient_tracker = GradientTrackerDoge(args.beta1, args.beta2, projector, projector_batch_size,
                                           len(mixture_weights))
    domain_weights_per_id = [1.0 for _ in range(len(mixture_weights))]
    domain_weights_per_id_list = [domain_weights_per_id]
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        for step, (batch, dataset_id) in enumerate(train_dataloader):
            ...
            loss = loss * domain_weights_per_id[dataset_id]
    ...
    # save domain_weights_per_id_list
    torch.save(domain_weights_per_id_list, os.path.join(args.output_dir, "domain_weights_per_id_list.pt"))
