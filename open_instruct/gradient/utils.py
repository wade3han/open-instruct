...
class GradientTracker:
            ...
            for i, dataset_id in enumerate(self.temporary_dataset_ids):
                ...
                if valid_num_batches is not None:  # validation grads
                    ...
                    if self.train_gradient_store_exp_avg.get(dataset_id) is None:
                        self.train_gradient_store_exp_avg[dataset_id] = projected_vectorized_grads[i]
                    else:
                        self.train_gradient_store_exp_avg[dataset_id] = \
                            self.beta1 * self.train_gradient_store_exp_avg[dataset_id] + \
                            (1 - self.beta1) * projected_vectorized_grads[i]
                    if self.train_gradient_store_exp_avg_sq.get(dataset_id) is None:
                        self.train_gradient_store_exp_avg_sq[dataset_id] = projected_vectorized_grads[i] ** 2
                    else:
                        self.train_gradient_store_exp_avg_sq[dataset_id] = \
                            self.beta2 * self.train_gradient_store_exp_avg_sq[dataset_id] + \
    ...
    def calc_sim(self, gradient_store_avg: dict[int, torch.tensor]):
        ...
        for train_id, val_id in itertools.product(range(num_train_set), range(num_valid_set)):
            try:
                train_grad = self.train_gradient_store_exp_avg[train_id] / torch.sqrt(
                    self.train_gradient_store_exp_avg_sq[train_id] + 1e-8)
            ...
            for i, dataset_id in enumerate(self.temporary_dataset_ids):
                ...
                if valid_num_batches is not None:  # validation grads
                    if self.eval_gradient_store_avg.get(dataset_id) is None:
                        ...
                        self.eval_gradient_store_avg[dataset_id] = \
                            self.beta1 * self.eval_gradient_store_avg[dataset_id] + \
                            (1 - self.beta1) * projected_vectorized_grads[i]
