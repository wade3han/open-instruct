...
class KarrasEMA(Module):
    ...
    def __init__(
        ...
        assert exists(sigma_rel) ^ exists(
            gamma), 'either sigma_rel or gamma is given. gamma is derived from sigma_rel as in the paper, then beta is dervied from gamma'
...
class PostHocEMA(Module):
    ...
    def __init__(
        ...
        assert len(
            gammas) > 1, 'at least 2 ema models with different gammas in order to synthesize new ema models of a different gamma'
    ...
    def checkpoint(self):
        ...
        for ind, ema_model in enumerate(self.ema_models):
            ...
            if self.save_func is None:
                pkg = deepcopy(ema_model.half()).to(self.checkpoint_dtype).state_dict()
                torch.save(pkg, str(path))
            else:
                self.save_func(ema_model.half(), path)
    ...
    def synthesize_ema_model(
        ...
        assert step <= max(
            timesteps), f'you can only synthesize for a timestep that is less than the max timestep {max(timesteps)}'
        ...
        for ind, (checkpoint, weight) in enumerate(zip(checkpoints, weights.tolist())):
            ...
            for ckpt_tensor, synth_tensor in zip(tmp_ema_model.iter_all_ema_params_and_buffers(),
